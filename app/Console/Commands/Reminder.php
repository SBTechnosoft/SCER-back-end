...
class Reminder extends Command
{
        ...
        if(count($settingData)!=0)
        {
            ...
            for($settingArrayData=0;$settingArrayData<$settingDataCount;$settingArrayData++)
            {
                if(strcmp($settingData[$settingArrayData]->setting_type,$constantArray['birthDateReminderSetting'])==0)
                {
                    ...
                    {
                        $birthString = "birth";
                        $result = $this->allReminder($decodedSettingData,$databaseName,$birthString,$resultDataArray);
                ...
                else if(strcmp($settingData[$settingArrayData]->setting_type,$constantArray['anniDateReminderSetting'])==0)
                {
                    ...
                    {
                        $anniString = "anni";
                        $result = $this->allReminder($decodedSettingData,$databaseName,$anniString,$resultDataArray);
    ...
    */
    public function allReminder($decodedSettingData,$databaseName,$birthAnni,$resultDataArray)
    {
        $constantDatabase = new ConstantClass();
        $constantArray = $constantDatabase->constantVariable();
        $commentMessage = $constantDatabase->getCommentMessage();
        //add days in date
        ...
        if(count($clientData)!=0)
        {
            ...
            if($diff->y==0 && $diff->m==0 && $diff->d==0) //&& $diff->h==0
            {
                ...
                if(strcmp($notifyBy,"sms")==0 || strcmp($notifyBy,"both")==0)
                {
                    ...
                    {
                        $type='sms';
                        //check mail is send or not to particular client
                        $conversationModel = new ConversationModel();
                        $result = $conversationModel->getExistingConversationData($clientData[$clientArrayData]->client_id,$commentMessage->reminderSmsSend,$type);
                        if(strcmp($result,$exceptionArray['500'])==0)
                        {
                            array_push($resultDataArray, $commentMessage->alreadySmsSend." to ".$clientData[$clientArrayData]->contact_no);
                            continue;
                        }
                        $htmlBody="";
                ...
                if(strcmp($notifyBy,"email")==0 || strcmp($notifyBy,"both")==0)
                {
                    ...
                    for($clientArrayData=0;$clientArrayData<$clientCount;$clientArrayData++)
                    {
                        ...
                        //check mail is send or not to particular client
                        $conversationModel = new ConversationModel();
                        $result = $conversationModel->getExistingConversationData($clientData[$clientArrayData]->client_id,$commentMessage->reminderMailSend,$type);
                        if(strcmp($result,$exceptionArray['500'])==0)
                        {
                            array_push($resultDataArray, $commentMessage->alreadyMailSend." to ".$clientData[$clientArrayData]->contact_no);
                            continue;
                        }
                        $htmlBody="";
                        $ledgerData = $ledgerModel->getDataAsPerClientId($clientData[$clientArrayData]->client_id);
