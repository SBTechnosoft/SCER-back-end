...
class ConversationProcessor extends BaseProcessor
{
	...
	{	
		...
		else
		{
			...
			if($status=="Success")
			{
				...
				$successFlag=0;
				//mail/sms send
				$result = $this->mailOrSmsSend($tRequest,$processedData,$request->header());
				if(!is_array($result))
				{
					return $result;
				}
     ...
     */	
	public function mailOrSmsSend($trimRequest,$documentData=null,$headerData)
	{
		//get exception message
		$exception = new ExceptionMessage();
			...
			if(array_key_exists('saleid',$headerData))
			{
				...
				if($headerData['saleid'][0]!='')
				{
					...
					if($decodedClientData->clientData[0]->email_id!='')
					{
						...
						$emailTemplateData = $templateService->getSpecificData($decodedBillData[0]->company_id,$templateArray['emailTemplate']);
			...
			else if(count($trimRequest['client_id'])!=0)
			{
				...
				for($clientArray=0;$clientArray<$clientIdCount;$clientArray++)
				{
					...
					else
					{
						...
						$commentDataArray[$commentIndex]['comment'] = $commentMessage->crmMailSend;
