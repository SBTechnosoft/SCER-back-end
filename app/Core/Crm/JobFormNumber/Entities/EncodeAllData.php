...
class EncodeAllData extends CompanyService
{
	public function getEncodedAllData($status)
	{
		...
		for($decodedData=0;$decodedData<count($decodedJson);$decodedData++)
		{
			...
			$jobCardNumberId[$decodedData] = $decodedJson[$decodedData]['job_card_number_id'];
			$jobCardNumberLabel[$decodedData] = $decodedJson[$decodedData]['job_card_number_label'];
			$jobCardNumberType[$decodedData] = $decodedJson[$decodedData]['job_card_number_type'];
			...
			$jobFormNumber->setCreated_at($convertedCreatedDate[$decodedData]);
			$getCreatedDate[$decodedData] = $jobFormNumber->getCreated_at();
			...
			else
			{
				...
				$jobFormNumber->setUpdated_at($convertedUpdatedDate[$decodedData]);
				$getUpdatedDate[$decodedData] = $jobFormNumber->getUpdated_at();
			}
		}
		$data = array();
